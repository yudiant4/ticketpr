...
// --- DATA DUMMY DENGAN EMOJI LENGKAP ---
const browseEvents = [
...
export default function BrowsePage() {
    ...
    return (
        ...
        .page-header { background: linear-gradient(135deg, var(--purple) 0%, var(--purple-light) 60%, var(--pink) 100%); padding: 48px; position: relative; overflow: hidden; margin-top: 72px; }
        ...
        .page-header h1 { font-size: 42px; font-weight: 800; color: white; letter-spacing: -0.02em; margin-bottom: 8px; }
        .page-header-sub { font-size: 15px; color: rgba(255,255,255,0.8); display: flex; align-items: center; gap: 16px; }
        .result-badge { background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3); border-radius: 50px; padding: 3px 12px; font-size: 13px; font-weight: 600; color: white; }

        .main-layout { max-width: 1300px; margin: 0 auto; padding: 32px 48px 80px; display: grid; grid-template-columns: 280px 1fr; gap: 28px; }
        .sidebar { background: white; border: 1px solid var(--border); border-radius: 20px; position: sticky; top: 100px; height: fit-content; }
        ...
        .filter-group-title { font-size: 13px; font-weight: 700; color: var(--text); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 14px; }
        
        .cat-tag { display: inline-flex; align-items: center; gap: 5px; padding: 6px 12px; border-radius: 50px; border: 1.5px solid var(--border); font-size: 12px; font-weight: 600; cursor: pointer; margin-right: 8px; margin-bottom: 8px; }
        .cat-tag.active { border-color: var(--purple); color: var(--purple); background: var(--bg3); }
        ...
        .view-btn { width: 34px; height: 34px; border: 1px solid var(--border); border-radius: 8px; background: white; cursor: pointer; color: var(--text3); }
        ...
        .event-card { background: white; border: 1px solid var(--border); border-radius: 18px; overflow: hidden; transition: 0.3s; }
        .event-card:hover { transform: translateY(-5px); box-shadow: 0 16px 48px var(--shadow); }
        
        .event-visual { height: 160px; position: relative; display: flex; align-items: center; justify-content: center; }
        .ev1 { background: linear-gradient(135deg, #667EEA, #764BA2); } .ev2 { background: linear-gradient(135deg, #F093FB, #F5576C); } .ev3 { background: linear-gradient(135deg, #4FACFE, #00F2FE); } 
        .ev4 { background: linear-gradient(135deg, #43E97B, #38F9D7); } .ev5 { background: linear-gradient(135deg, #FA709A, #FEE140); } .ev6 { background: linear-gradient(135deg, #A18CD1, #FBC2EB); }
        .ev7 { background: linear-gradient(135deg, #f7971e, #ffd200); } .ev8 { background: linear-gradient(135deg, #11998e, #38ef7d); } .ev9 { background: linear-gradient(135deg, #ee0979, #ff6a00); }
        
        .event-emoji { font-size: 56px; filter: drop-shadow(0 6px 12px rgba(0,0,0,0.2)); }
        .event-status { position: absolute; top: 12px; left: 12px; font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 50px; }
        ...
        .event-wishlist { position: absolute; bottom: 12px; right: 12px; width: 32px; height: 32px; background: rgba(255,255,255,0.9); border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; }
        .event-wishlist.liked { background: #FEE2E2; }
        
        .event-body { padding: 18px; }
        .event-name { font-size: 15px; font-weight: 700; margin-bottom: 5px; }
        .event-org { font-size: 12px; color: var(--purple); font-weight: 600; margin-bottom: 10px; }
        .event-tag { font-size: 11px; font-weight: 600; padding: 3px 8px; border-radius: 50px; background: var(--bg3); color: var(--purple); margin-right: 5px; }
        
        .supply-label { display: flex; justify-content: space-between; font-size: 11px; color: var(--text3); margin-bottom: 5px; }
      ...
      `}} />
            ...
            <div className="main-layout">
                <aside className="sidebar">
                    ...
                    <div className="filter-group">
                        <div className="filter-group-title">Blockchain ⛓️</div>
                        <div style={{ fontSize: '13px', padding: '5px 0' }}>✅ Ethereum</div>
                        <div style={{ fontSize: '13px', padding: '5px 0' }}>✅ Polygon</div>
                        <div style={{ fontSize: '13px', padding: '5px 0' }}>⬜ Base</div>
                    </div>
                    <div className="filter-group">
                        <button style={{ width: '100%', padding: '12px', background: 'linear-gradient(135deg, #7C3AED, #A855F7)', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 700, cursor: 'pointer' }}>Apply Filters ⚡</button>
                    </div>
                </aside>
                ...
                <div className="content-area">
                    <div className="sort-bar">
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                            ...
                            <select style={{ border: '1px solid #E8E4F5', borderRadius: '8px', padding: '6px 10px', fontSize: '13px', fontWeight: 600 }}>
                        ...
                        {browseEvents.map((ev) => (
                            <div key={ev.id} className="event-card">
                                <div className={`event-visual ${ev.bgClass}`}>
                                    ...
                                    <span className="event-chain">{ev.chain} ⛓️</span>
                                    <div className="event-emoji">{ev.emoji}</div>
                                    <div className={`event-wishlist ${wishlist.includes(ev.id) ? 'liked' : ''}`} onClick={() => toggleWish(ev.id)}>
                                ...
                                <div className="event-body">
                                    ...
                                    <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: '#9896B0', marginBottom: '14px' }}>
                                    ...
                                    <div className="supply-bar">
                                        <div className="supply-label"><span>Tickets Sold 🎫</span><span>{ev.sold}/{ev.total}</span></div>
