...
export default function DashboardLayout() {
    ...
    const SidebarItem = ({ label, tabId, isLink = false, linkUrl = "/" }: any) => {
        const isActive = activeTab === tabId;
        const handleClick = () => { if (!isLink) { setActiveTab(tabId); if (isMobile) setIsMenuOpen(false); } }
        if (isLink) {
            return (
                <Link href={linkUrl} style={{ display: 'block', padding: '12px 16px', borderRadius: '12px', textDecoration: 'none', color: '#4B4869', fontWeight: 600, marginBottom: '4px' }}>
                    <span style={{ fontSize: '14px' }}>{label}</span>
                </Link>
            )
        }
        return (
            <button onClick={handleClick} style={{ display: 'block', padding: '12px 16px', borderRadius: '12px', width: '100%', border: 'none', background: isActive ? '#F3F0FF' : 'transparent', color: isActive ? '#7C3AED' : '#4B4869', fontWeight: isActive ? 700 : 600, cursor: 'pointer', marginBottom: '4px', textAlign: 'left', fontFamily: 'inherit' }}>
                <span style={{ fontSize: '14px' }}>{label}</span>
            </button>
        )
    }
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: '#FAFAFF', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
            ...
            <aside style={{ width: '260px', background: 'white', borderRight: '1px solid #E8E4F5', display: (isMobile && !isMenuOpen) ? 'none' : 'flex', flexDirection: 'column', position: 'fixed', height: '100vh', top: 0, left: 0, zIndex: 1000 }}>
                ...
                <div style={{ padding: '20px 16px', flex: 1, overflowY: 'auto' }}>
                    ...
                    <div style={{ fontSize: '11px', fontWeight: 700, color: '#9896B0', margin: '24px 0 12px', paddingLeft: '16px', letterSpacing: '1px' }}>EXPLORE</div>
                    <SidebarItem label="Buy Tickets" isLink={true} linkUrl="/market" />
                    <SidebarItem label="Verify Scanner" isLink={true} linkUrl="/verify" />

                    <div style={{ fontSize: '11px', fontWeight: 700, color: '#9896B0', margin: '24px 0 12px', paddingLeft: '16px', letterSpacing: '1px' }}>ACCOUNT</div>
                    <SidebarItem label="Profile" tabId="profile" />
                    <SidebarItem label="Settings" tabId="settings" />
                </div>
            </aside>
            ...
            <main style={{ flex: 1, marginLeft: isMobile ? '0' : '260px', padding: isMobile ? '20px' : '40px' }}>
                ...
                {activeTab === 'upcoming' && (
                    <div>
                        <div style={{ marginBottom: '30px' }}>
                            <h2 style={{ fontSize: '24px', fontWeight: 800 }}>My NFT Collection</h2>
                    ...
                    <div style={{ maxWidth: '800px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                            <div style={{ background: 'linear-gradient(135deg, #10B981, #059669)', borderRadius: '24px', padding: '30px', color: 'white', boxShadow: '0 10px 30px rgba(16, 185, 129, 0.2)' }}>
                                ...
                                <button onClick={() => alert('Smart Contract withdrawal initiated')} style={{ marginTop: '24px', width: '100%', background: 'white', color: '#059669', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: 800, cursor: 'pointer' }}>Withdraw Funds</button>
                            </div>
                ...
                {activeTab === 'settings' && (
                    <div style={{ background: 'white', padding: '40px', borderRadius: '24px', border: '1px solid #E8E4F5', maxWidth: '600px' }}>
                        <h2 style={{ fontWeight: 800, marginBottom: '30px' }}>Web3 App Settings</h2>
