...
export default function MarketPage() {
  ...
  // PERBAIKAN 1: Panggil mintTicket, bukan mint
  const { mintTicket, isPending } = useMintTicket()

  // PERBAIKAN 2: PENGAMAN ANTI-BLANK (Mencegah crash jika events bukan array)
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
        {isLoading ? (
          ...
          <div style={{ padding: '20px', background: '#FEE2E2', color: '#DC2626', borderRadius: '12px', gridColumn: '1 / -1' }}>
            <b>Info:</b> Tidak ada event yang ditemukan atau fungsi <code>getAllEvents()</code> tidak ada di Smart Contract-mu.
          </div>
          ...
          <div key={event.id?.toString()} style={{ background: 'white', padding: '24px', borderRadius: '24px', border: '1px solid #E8E4F5', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              ...
              <button
                // PERBAIKAN 3: Gunakan mintTicket di sini
